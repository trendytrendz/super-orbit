...
def create_pan_zoom_clip(duration, bg_path, size):
    ...
    try:
        ...
        if img_aspect > target_aspect:
            new_width = int(target_aspect * img.height); offset = (img.width - new_width) / 2
            img = img.crop((offset, 0, img.width - offset, img.height))
        else:
            new_height = int(img.width / target_aspect); offset = (img.height - new_height) / 2
...
def make_video(slides, audio_paths, company, nse_symbol, video_format, out_path, assets, theme, icon_svg):
    ...
    for slide_info in slides:
        key = slide_info['key']
        if key not in audio_paths or not os.path.exists(audio_paths[key]):
            print(f"    - Skipping slide '{key}' because audio is missing.")
            continue
        try:
            duration = AudioFileClip(audio_paths[key]).duration + 1.0
        except Exception as e:
            print(f"    - WARNING: Could not read audio duration for '{key}'. Skipping slide. Error: {e}"); continue
        ...
        if slide_info['type'] == 'intro':
            ...
            icon_path = os.path.join(utils.get_script_dir(), "outputs", "tmp", f"{slide_info['icon']}_icon.png")
            cairosvg.svg2png(bytestring=icon_svg[slide_info['icon']], write_to=icon_path, output_height=60)
            icon_clip = ImageClip(icon_path).set_duration(duration).set_position((VIDEO_W * 0.1, VIDEO_H * 0.15))
            text_clip = create_typing_text_clip(slide_info['text'], duration, font_path, 108, (VIDEO_W * 0.85, VIDEO_H * 0.8), ('center', 'center'), theme['accent'])
            ...
            if slide_info.get('path') and os.path.exists(slide_info['path']):
                ...
                print(f"    - WARNING: Chart path for slide '{key}' not found. Skipping slide visuals."); fg_clip = ColorClip(size=(1,1), color=(0,0,0,0), duration=duration)
    ...
    # *** CRITICAL FIX: Corrected codec name from 'libx24' to 'libx264' ***
