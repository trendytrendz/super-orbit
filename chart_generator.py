...
def make_comparison_bar_chart(metric_name, value_a, value_b, name_a, name_b, size, theme, out_png="outputs/tmp/comparison.png"):
    ...
    ax.patch.set_alpha(0)
    try:
        val_a_float = float(str(value_a).replace(",", "")) if value_a else 0
        val_b_float = float(str(value_b).replace(",", "")) if value_b else 0
    except (ValueError, TypeError):
        val_a_float, val_b_float = 0, 0
    labels = [name_a, name_b]
    values = [val_a_float, val_b_float]
    colors = [theme['accent'], '#B0BEC5']
    bars = ax.bar(labels, values, color=colors)
    for bar in bars:
        yval = bar.get_height()
        label_text = f'{yval:,.2f}'
        text_obj = ax.text(bar.get_x() + bar.get_width()/2.0, yval, label_text, va='bottom', ha='center', color=CHART_TEXT_COLOR, weight='bold')
        text_obj.set_path_effects(TEXT_EFFECT)
    ax.set_ylabel(metric_name, color=CHART_TEXT_COLOR, path_effects=TEXT_EFFECT)
    ax.set_title(f"Comparison: {metric_name}", color=CHART_TEXT_COLOR, fontsize=18, path_effects=TEXT_EFFECT)
...
def make_stock_vs_stock_price_chart(df_a, df_b, name_a, name_b, size, theme, out_png="outputs/tmp/stock_price_comp.png"):
    ...
    try:
        df_merged = pd.concat([df_a['Close'], df_b['Close']], axis=1, keys=[name_a, name_b]).dropna()
        df_norm = (df_merged / df_merged.iloc[0]) * 100
        plt.style.use('dark_background')
        fig, ax = plt.subplots(figsize=(size[0]/100, size[1]/100), dpi=100)
        fig.patch.set_alpha(0)
        ax.patch.set_alpha(0)
        ax.plot(df_norm.index, df_norm[name_a], color=theme['accent'], label=name_a, linewidth=2.5, path_effects=TEXT_EFFECT)
        ax.plot(df_norm.index, df_norm[name_b], color=CHART_TEXT_COLOR, label=name_b, linewidth=1.5, linestyle='--')
        ax.set_title(f"{name_a} vs. {name_b} (1-Year Performance)", color=CHART_TEXT_COLOR, fontsize=18, path_effects=TEXT_EFFECT)
        ...
        print(f"      - Could not generate stock vs stock price chart: {e}")
        return None
...
def make_shareholding_chart(data, size, theme, out_png="outputs/tmp/shareholding.png"):
    ...
    try:
        ...
        for text in legend.get_texts():
            text.set_color(CHART_TEXT_COLOR)
            text.set_path_effects(TEXT_EFFECT)
        legend.get_title().set_color(CHART_TEXT_COLOR)
        legend.get_title().set_path_effects(TEXT_EFFECT)
...
def make_metrics_infographic(metrics, size, theme, out_png="outputs/tmp/metrics.png"):
    ...
    ax.patch.set_alpha(0)
    ax.axis('off')
    title = ax.text(0.5, 0.9, "Key Metrics", color=theme['accent'], fontsize=36, weight='bold', ha='center', transform=ax.transAxes)
    title.set_path_effects(TEXT_EFFECT)
    metrics_list = list(metrics.items())
    y_start = 0.75
    for i, (key, value) in enumerate(metrics_list):
        y_pos = y_start - i * 0.12
        key_text = ax.text(0.1, y_pos, key, color=CHART_TEXT_COLOR, alpha=0.9, fontsize=20, ha='left', va='center', transform=ax.transAxes)
        key_text.set_path_effects(TEXT_EFFECT)
        value_text = ax.text(0.9, y_pos, str(value), color=theme['accent'], fontsize=28, weight='bold', ha='right', va='center', transform=ax.transAxes)
